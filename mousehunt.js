...
function bwRift() {
    ...
    var objBWRift = {
        ...
        priorities: ['SECURITY', 'FURNACE', 'PURSUER', 'LUCKY', 'HIDDEN', 'GUARD', 'FROZEN', 'ACOLYTE',  'TIMEWARP', 'ANCIENT', 'RUNIC', 'GEARWORKS', 'INGRESS'],
        prioritiesCursed: ['SECURITY', 'FURNACE', 'PURSUER', 'LUCKY', 'HIDDEN', 'ANCIENT', 'GEARWORKS', 'RUNIC', 'TIMEWARP', 'GUARD', 'FROZEN', 'ACOLYTE', 'INGRESS'],
        minTimeSand: [70, 70, 50, 50, 50, 50, 50, 50, 999],
        minRSCType: 'GEQ',
        minRSC: 0,
    ...
    // Ancient Lab
    objBWRift.master.trinket[2] = "None";

    // Runic lab
    objBWRift.master.trinket[3] = "None";
    objBWRift.master.bait[3] = "Ancient String";

    // Timewarp Chamber
    //objBWRift.master.trinket[4] = "None";
    objBWRift.master.bait[4] = "Runic String";

    // Guard Barracks
    objBWRift.master.bait[5] = "Runic String";
    objBWRift.master.activate[5] = true;

    // Frozen alcove
    objBWRift.master.bait[7] = "Runic String";
    objBWRift.master.activate[7] = true;

    // Ingress
    objBWRift.master.bait[9] = "Runic String";
    objBWRift.master.activate[9] = true;

    // Acolyte charging
    objBWRift.master.bait[11] = "Runic String";
    objBWRift.master.activate[11] = true;

    // Acolyte draining
    objBWRift.master.bait[12] = "Runic String";

    // Acolyte drained
    objBWRift.master.bait[13] = "Runic String";
    objBWRift.master.activate[13] = true;

    // Lucky Tower
    objBWRift.master.activate[14] = true;
    objBWRift.master.trinket[14] = "None";
...
function bountifulBeanstalk() {
    ...
    var toggleFuelOn = (isGreatHallFloor && lootMultiplier >= 8) || (lootMultiplier >= 8 && isCastle && roomType == "ha");
...
//         }
        if (lootMultiplier >= 8) {
            ...
            if (lootMultiplier < nextRoomLootMultiplier && !objUser.castle.is_boss_chase) {
                ...
                if (objUser.items.royal_beanster_cheese.quantity_unformatted > 1 && (objUser.castle.noise_level < objUser.castle.max_noise_level) && !isCastleBossEncounter) {
                    checkThenArm(null, 'bait', 'Royal Beanster Cheese');
                    checkThenArm(null, 'trinket', 'Rift Charm');
                    if (!fuelOn) {
                        fireEvent(fuelButton, 'click');
...
//         }
        ...
        if (cropCoinNumber >= 300 && mythicalMulchNumber >= 60) {
        ...
        } else if (mythicalMulchNumber >= 60) {
            if (parseInt(magicEssenceNumber.replace(/,/g, '')) >= 9 && parseInt(papyrusSeedNumber.replace(/,/g, '')) >= 30) {
                for (let i = 0; i < 3; i++) {
                    if (!(objUser.plots[i].can_plant_anything && !objUser.plots[i].is_growing)) {
                        break;
	    ...
	    disarmTrap('trinket');
            ...
            for (let i = 0; i < 3; i++) {
                if (objUser.plots[i].can_plant_anything && !objUser.plots[i].is_growing) {
                    ...
                    if (parseInt(magicEssenceNumber.replace(/,/g, '')) >= 3 && parseInt(papyrusSeedNumber, 10) >= 10) {
        ...
        if (showTimerInPage) {
            ...
            if (headerElement) {
                ...
                if (targetPage) {
                    ...
                    //text = document.createTextNode('Let me know if you know how to remove this button!');
    ...
    if (doubleCheckLocation()) {
        ...
        if (!aggressiveMode) {
            ...
            if (headerElement) {
                ...
                if (headerStatus.checkVisibility()) {
